export const GLOSSARY: Record<string, { title: string; body: string }> = {
  ...
  reconcile_mode: {
    title: "Reconcile — Predicted vs Actual",
    body: "The optimizer runs backtests on historical data — simulating what would have happened if the bot had been trading over the past year. Those simulations make assumptions: that you got filled at the mid-price, no slippage, market moved exactly as history shows.\n\nAfter you've been running in paper mode for a few weeks, you have two sets of data: what the backtest predicted, and what actually happened in real market conditions.\n\nReconcile compares them side by side — win rate predicted vs actual, premium collected predicted vs actual, P&L predicted vs actual — and produces a model accuracy score.\n\nIf real results closely match the backtest, the model is trustworthy and you can have more confidence going live. If there's a big gap (e.g. backtest predicted 25% annual yield but paper trading shows 10%), the assumptions were too optimistic and you'd want to investigate before using real money.\n\nThis feature requires a few weeks of paper trade history to be meaningful. It becomes your quality control check before ever switching to live money.",
  },
  regime_filter: {
    title: "Regime Filter",
